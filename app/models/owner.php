...
class Owner extends User
{
	...
	public function __construct()
	{
		...
		if ($this->isLoggedIn() === true) {
			$tmpOwner = $this->_findByUID($this->_getPrivateData('userID'));
			if (is_null($tmpOwner) === false and is_object($tmpOwner) === true) {
				if (isset($tmpOwner->status) === true and $tmpOwner->status === true and isset($tmpOwner->data) === true and is_null($tmpOwner->data) === false and isset($tmpOwner->data->store) === true) {
					$this->_store 		= new Store($tmpOwner->data->store);
	...
	private function _findByUID($userID = null, $type = null)
	{
		...
		if (is_null($userID) === false) {
			...
			$findOwner = $this->_db->get('`owner_id`, `store_id`', 'owners', '`user_id` = ? AND `owner_status` = ?', array($userID, $type));
			if ($findOwner->errorStatus() === false and $findOwner->dataCount() == 1) {
				$tmpOwner = $findOwner->getFirstResult();
				$tmpResponse->status = true;
				$tmpResponse->data = (object)[
					'id'				=> $tmpOwner->owner_id,
					'store'			=> $tmpOwner->store_id,
				];
				return $tmpResponse;
			} else {
	...
	public function register($data = null)
	{
		...
		if ($this->_parseRegisterData($data) === true) {
			// data parsed successfully
			if ($this->_isEmailUnique($data->ownerEmail) === true) {
				...
				$createUser = $this->_db->insert('users', array(
					...
					'password'		=> Hash::make($data->ownerPassword, Hash::make($tmpUserID)),
					'role_id'			=> '6ac83974f21c0bb0d8983575cdd022ac'
				));
				if ($createUser->errorStatus() === false) {
					$storeRegistration = $this->_registerStore((object)[
						'storeName'		=> $data->storeName
					]);
					if ($storeRegistration !== false and is_object($storeRegistration) === true) {
						$storeRegistration = (object)$storeRegistration;
						if (isset($storeRegistration->status) === true and isset($storeRegistration->data) === true and isset($storeRegistration->data->storeID) === true) {
							if ($this->registerBranch((object)[
								'name'				=> 'Main Branch',
								'description'	=> 'This is the main branch.'
							], null, $storeRegistration->data->storeID) === false) {
								$this->setError($dem);
								return false;
							}
							$tmpOwnerID = Hash::unique();
							$createOwner = $this->_db->insert('owners', array(
								'owner_id'			=> $tmpOwnerID,
								'user_id'				=> $tmpUserID,
								'store_id'			=> $storeRegistration->data->storeID,
								'owner_status'	=> 1
							));
							if ($createOwner->errorStatus() === false) {
								$activateUserAccount = $this->_db->update('users', '`account_status` = ?', '`user_id` = ?', array(1, $tmpUserID));
								if ($activateUserAccount->errorStatus() === false) {
									$tmpResponse = (object)[
										'status'			=> false,
										'type'				=> 0,
										'message'			=> null
									...
									$this->setError('Failed to activate your account. You could not be able to login. Please contact with admin.');
									return false;
								}
							} else {
								$this->setError('Store registration failed. Please try again later or contact with admin.');
								return false;
							}
						} else {
							$this->setError($dem);
					...
					$this->_db->delete('users', '`user_id` = ?', array($tmpUserID));
					$this->setError($dem);
					return false;
				}
			} else {
				// email is not unique, error is already set thorough parent class
				return false;
			}
		} else {
			$this->setError($dem);
	...
	// store
	private function _registerStore($data = null)
	{
		$dem = 'Failed to register store.';
		if (is_null($data) === false and $this->_parseStoreData($data) === true) {
			$tmpStoreID = Hash::unique();
			$registerStore = $this->_db->insert('stores', array(
				'store_id'     	=> $tmpStoreID,
				'name'          => $data->storeName,
				'store_status' 	=> 1
			));
			if ($registerStore->errorStatus() === false) {
				return (object)[
					'status'    => true,
					'data'      => (object)[
						'storeID'    => $tmpStoreID
	...
	public function addStoreItem($basicDetails = null, $options = null)
	{
		...
		$options = array();
		switch (strtoupper($basicDetails->itemCategory)) {
			case 'COSMETICS':
				$options = array('Weight/Size');
				break;
			case 'PHARMACY':
				$options = array('Potency');
				break;
			case 'GROCERY':
				$options = array('Weight/Size');
				break;
			...
			if (isset($storeItem->status) === true and $storeItem->status === true) {
				...
				if (is_null($storeItem->data) === false and is_array($storeItem->data) === true and count($storeItem->data) == 1) {
					...
					if (isset($storeItem->data) === false or is_null($storeItem->data) === true or is_array($storeItem->data) === false and count($storeItem->data) != 1) {
						...
						if (empty($basicDetails->itemGeneral) === true) {
							$basicDetails->itemGeneral = null;
						}
						$addStoreItem = $this->_db->insert('store_items', array(
							...
							'general_salt'	=> $basicDetails->itemGeneral,
							'box_units'			=> $basicDetails->itemUnits,
	...
	public function addVariants($storeItemID = null, $variants = null)
	{
		...
		if (is_null($storeItemID) === false and is_array($variants) === true and count($variants) > 0) {
			...
			if (is_null($itemOptions) === false and is_array($itemOptions) === true and count($itemOptions) > 0) {
				...
				Redirect::to('./dashboard/item/view/' . $storeItemID);
				exit();
				return true;
			} else {
				$this->setError($dem);
	...
	public function editStoreItem($storeItemID = null, $basicDetails = null)
	{
		...
		if ($this->isLoggedIn() === true and is_null($storeItemID) === false and $this->__parseItemBasicDetails($basicDetails) === true) {
			...
			if (isset($storeItem->data) === false or is_null($storeItem->data) === true or is_array($storeItem->data) === false and count($storeItem->data) != 1 and isset($storeItem->data[0]->id) === true) {
				...
				$updateBasicDetails = $this->_db->update('store_items', '`item_id` = ?, `category_id` = ?, `brand_id` = ?, `general_salt` = ?', '`si_id` = ?', array($item->data->id, $category->data->id, $brand->data->id, $basicDetails->itemGeneral, $storeItemID));
	...
	public function saleStart($pCart = null)
	{
		...
		if ($this->__parseSaleCartItems($pCart) === true) {
			...
			$validatedItems = array();
			$netTotal = 0;
			$stockCheck = $this->_store->getStockCheck();
			foreach ($pCart as $item) {
				$item = json_decode($item);
				// check its stock
				$stock = $this->_storeItem->getVariantStock($item->saleItemID);
				if ($stockCheck === true and $item->saleQuantity > $stock) {
					$this->setError($dem . " Stock of <b>{$item->saleItem}</b> is not enough.");
					return false;
				}
				array_push($validatedItems, (object)[
	...
	public function saleConfirm($details = null, $queueStatus = false)
	{
							...
							if ($createTransaction->errorStatus() === false) {
								...
								if ($addSalePayment->errorStatus() === false) {
									...
									if ($updatePaymentStatus->errorStatus() === false) {
										$msgPre = ($queueStatus === false) ? 'Sale completed' : 'Order Queued';
										if ((Input::roundMoney($tmpCart->bill->netTotal) - Input::roundMoney($saleDiscount)) == Input::roundMoney($details['salePaid'])) {
											Session::flash('successMessage', $msgPre . ' successfully.');
											Redirect::to('./dashboard/sale/receipt/' . $tmpSaleID);
											exit();
										} else {
											Session::flash('successMessage', $msgPre . ' successfully. However, a few amount is due from net total.');
											Redirect::to('./dashboard/sale/receipt/' . $tmpSaleID);
											exit();
										}
									} else {
	...
	public function getQueue($params = null)
	{
		if ($this->isLoggedIn() === true) {
			return $this->_sale->getQueue($this->getStoreData('id'), $params);
		} else {
			return null;
		}
	}
	...
	public function getCustomSaleReport($customer = null, $brand = null, $dateRange = null, $type = null)
	{
		$dem = 'Purchase report not found.';
		if (is_null($dateRange) === false and empty($dateRange) === false) {
			$dateRange = explode('-', $dateRange);
			array_walk($dateRange, function (&$value, $key) {
				$value = trim($value);
			});
			$dateRange = (object)[
		...
		if ($this->isLoggedIn() === true) {
			$saleReport = null;
			if (is_null($customer) === false or is_null($brand) === false) {
				$saleReport = $this->_sale->getCustomReport($this->getStoreData('id'), $customer, $brand, $dateRange);
				if ($saleReport !== false and is_null($saleReport) === false) {
					return $saleReport;
				} else {
					$this->setError($dem);
					return null;
	...
	public function updateBuyer($buyerID = null, $buyerData = null)
	{
		$dem = 'Failed to update customer profile.';
		if (is_string($buyerID) === true and is_null($buyerData) === false and is_object($buyerData) === true) {
			$update = $this->_db->update('buyers', '`name` = ?, `contact` = ?, `area` = ?', '`buyer_id` = ?', array($buyerData->name, $buyerData->contact, $buyerData->area, $buyerID));
	...
	{
		$dem = 'Brands not found.';
		$brands = $this->_storeItem->getBrands($this->getStoreData('id'));
		if (is_array($brands) === true) {
			return $brands;
		} else {
			$this->setError($dem);
			return null;
